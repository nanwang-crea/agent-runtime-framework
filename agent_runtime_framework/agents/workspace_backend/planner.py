...
def infer_task_intent(user_input: str, workspace_root: Path | None = None) -> TaskIntent:
    ...
    if (any(token in text for token in _DIR_HINTS) or any(token in lowered for token in _DIR_HINTS) or (any(token in text for token in _LIST_HINTS) and any(token in text for token in ("工作区", "文件", "目录", "仓库")))):
        if target_hint and (any(token in text for token in _READ_HINTS) or any(token in lowered for token in _READ_HINTS)):
            return TaskIntent(task_kind="compound_read", user_intent="compound_read", target_hint=target_hint)
        return TaskIntent(task_kind="workspace_discovery", user_intent="workspace_discovery", target_hint=target_hint)
    if target_hint or any(token in text for token in _READ_HINTS) or any(token in lowered for token in _READ_HINTS):
        compound_markers = ("并", "以及", "同时", "and")
        if target_hint and any(marker in lowered or marker in text for marker in compound_markers):
            return TaskIntent(task_kind="compound_read", user_intent="compound_read", target_hint=target_hint)
        return TaskIntent(task_kind="workspace_read", user_intent="workspace_read", target_hint=target_hint)
    return TaskIntent(task_kind="chat", user_intent="conversation")
...
def _plan_from_goal(user_input: str, *, tool_names: set[str]) -> WorkspaceAction | None:
    ...
    if intent.task_kind == "workspace_discovery" and target_hint and "inspect_workspace_path" in tool_names:
