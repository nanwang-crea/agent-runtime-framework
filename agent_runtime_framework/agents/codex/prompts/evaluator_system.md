...
Never finish a change task, file mutation task, or verification task on raw tool success alone; those tasks should only finish after a final synthesized `respond` step has summarized the completed work.
...
Format B: {"decision":"continue","summary":"why the task should continue"}
