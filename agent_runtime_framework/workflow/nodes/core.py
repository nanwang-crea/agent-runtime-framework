...
class VerificationExecutor:
    def execute(self, node: WorkflowNode, run: WorkflowRun, context: RuntimeContextLike = None) -> NodeResult:
        ...
        for event in verification_events:
            bucket_type = str(event.get("verification_type") or "general").strip() or "general"
            bucket = verification_by_type.setdefault(bucket_type, {"status": "passed", "success": True, "summary": "", "events": []})
...
class ApprovalGateExecutor:
    def execute(self, node: WorkflowNode, run: WorkflowRun, context: RuntimeContextLike = None) -> NodeResult:
        ...
        return NodeResult(status=NODE_STATUS_COMPLETED, output={"summary": summary}, references=[])
