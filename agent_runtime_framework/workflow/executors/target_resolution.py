...
from agent_runtime_framework.workflow.state.models import InteractionRequest, NODE_STATUS_COMPLETED, NODE_STATUS_FAILED, NodeResult, WorkflowNode, WorkflowRun
    ...
    def execute(self, node: WorkflowNode, run: WorkflowRun, context: RuntimeContextLike = None) -> NodeResult:
        ...
        if str(output.get("resolution_status") or "") in {"ambiguous", "unresolved"}:
            ...
            interaction_request = InteractionRequest(
                kind="clarification",
                prompt=prompt,
                summary=str(output.get("summary") or prompt),
                items=[str(item) for item in output.get("items", []) or [] if str(item).strip()],
