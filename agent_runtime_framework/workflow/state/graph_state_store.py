        ...
        for item in prior_state.get("judge_history", []) or []:
            state.judge_history.append(
                JudgeDecision(
                    ...
                    preferred_recipe_ids=[str(v) for v in item.get("preferred_recipe_ids", []) or [] if str(v).strip()],
                    blocked_recipe_ids=[str(v) for v in item.get("blocked_recipe_ids", []) or [] if str(v).strip()],
                    must_cover_capabilities=[str(v) for v in item.get("must_cover_capabilities", []) or [] if str(v).strip()],
